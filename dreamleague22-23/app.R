...
# preprocessing
dl=googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1Rg7qc_DOyyAn5dIG5KbDVguTPx2h4zDMxB8y5kQfmHQ/edit#gid=0",
                             na=c("SOLD",""),
                             col_names = F)

teams=dl %>% dplyr::select(1:7) %>% 
  rename(position=1,
         player=2,
         club=3,
         cost=4,
...
managers=dl %>% dplyr::select(10:11) %>% 
  na.omit() %>% 
  rename(manager=1,
         team=2) %>% 
  filter(team!="TEAM")
...
server <- function(input, output) {
  ...
  output$team=renderDataTable({
    ...
    if(input$current)
    {
      teams3= teams2%>% filter(team==input$team)  %>% filter(is.na(sold)) %>% select(-sold)
    }else
    {
  ...
  output$teamtext=renderUI({
    ...
    outfield=paste("Outfield transfers remaining:", 8-teams2 %>% filter(team==input$team, position!= "GOALKEEPER", cost=="TRANSFER") %>% nrow())
    goalie=paste("Goalkeeper transfers remaining:", 2-teams2 %>% filter(team==input$team, position== "GOALKEEPER", cost=="TRANSFER") %>% nrow())
