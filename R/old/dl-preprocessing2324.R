...
tictoc::tic()
dl = readxl::read_excel(
  "data/DreamLeague23-24.xlsx",
  na = c("SOLD"),
  sheet = "Stats"
)
gs4_auth(
...
teams3 = teams2 %>%
  ...
  mutate(
    ...
    player = case_when(
      player == "DANIEL ORSI" ~ "DANILO ORSI-DADAMO",
...
player_id2 = player_id %>%
  ...
  rbind.data.frame(data.frame(
    "player" = c("MIKEY JOHNSTON"),
    "player_id" = c(93317),
    "team" = c("WEST BROMWICH ALBION")
  ))
...
outfield0 = teams3 %>%
  ...
  mutate(
    ...
    sold2 = as.Date(
      case_when(
        grepl("Jul|Aug|Sep|Oct|Nov|Dec", sold) ~ paste(sold, "-2023", sep = ""),
  ...
  tryCatch(
    {
      ...
      appgoals = (tables$tpg) %>%
        ...
        filter(Date > outfield$bought2[i], Date < outfield$sold2[i]) %>%
        mutate(
          Goals = as.numeric(V7),
          App = 1,
...
#
test = outfield %>%
  filter(goals != SBgoals, !is.na(id)) %>%
  select(position, player, goals, SBgoals, player_id) %>%
  rename("BCgoals" = "goals")
# load("Data/team_id.RDa")

gk = teams3 %>%
  filter(position %notin% c("DEFENDER", "MIDFIELDER", "FORWARD")) %>%
  merge(
    team_id %>%
      select(team, team_id) %>%
      mutate(
        team = if_else(team == "WEST BROMWICH ALBION", "WEST BROM", team)
      ) %>%
      group_by(team) %>%
  ...
  tryCatch(
    {
      ...
      x = sl %>%
        group_by(row_number()) %>%
        mutate(
          comp = str_extract(link, lg),
          date = as.Date(
            str_extract(link, "\\d{4}-([0]\\d|1[0-2])-([0-2]\\d|3[01])"),
            "%Y-%m-%d"
          ),
        ...
        mutate(
          link = gsub("Bristol C", "Bristolc", link),
        ...
        mutate(
          teampos = unlist(gregexpr(
            "[a-z][[:space:]]1[[:space:]]\\d{4}-([0]\\d|1[0-2])-([0-2]\\d|3[01])",
            link
          ))[1],
          opppos = unlist(gregexpr(
            "[a-z][[:space:]]2[[:space:]]\\d{4}-([0]\\d|1[0-2])-([0-2]\\d|3[01])",
            link
          ))[1]
        ) %>%
        ungroup() %>%
        ...
        mutate(
          concede = -as.numeric(str_trim(if_else(teampos > opppos, H, A)))
        ) %>%
        filter(
          comp %in%
            c(
        ...
        filter(date > gk$bought2[i], date < gk$sold2[i], !is.na(H))
...
sheet_write(
  team_score,
  ss = "https://docs.google.com/spreadsheets/d/1dKUl4hpZ0SnqqLoZk5IpJwISKoMj7o0WNoeUoLebc8s/edit#gid=0",
  sheet = "scores"
)
...
team_score_weekly = rbind.data.frame(
  ...
  summarise(SBgoals = sum(Goals, na.rm = T), App = sum(App, na.rm = T)) %>%
