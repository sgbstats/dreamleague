...
gk=read.xlsx("data/DL-DATA2025.xlsx", sheet="Goalkeepers")
def=read.xlsx("data/DL-DATA2025.xlsx", sheet="Defenders")
mid=read.xlsx("data/DL-DATA2025.xlsx", sheet="Midfielders")
forwards=read.xlsx("data/DL-DATA2025.xlsx", sheet="Forwards")
...
data3=data2%>% 
  ...
  merge(gk %>% janitor::clean_names() %>% dplyr::select(-total, -pos) %>% 
          rename("team"="club"), by="team") %>% 
  dplyr::select(num,pos, player.x, player.y, club, team,league, total, player_id,dist, check_needed ) %>% 
  arrange(num)
  ...
  # for(i in 1:100)
{
  ...
  tryCatch({
    ...
    appgoals=(tables$tpg) %>% filter(V1 %in% c("English premier", "English Premier","Premier League",
...
data1=union_all(def, mid) %>% union(forwards) %>% janitor::clean_names() %>% mutate(rn=row_number())
data2=player_id2  %>% filter(position!="Goalkeeper")%>% 
  fuzzyjoin::stringdist_join(data1 %>% mutate(player=str_to_upper(player)),
                             by="player", mode="left", method="jw", distance_col="dist") %>% 
  ungroup() %>% 
  slice_min(order_by=dist, n=1, by="player.x")
data3 =data2%>% filter(SBgoals>0, position!="Goalkeeper") %>% 
  mutate(position=factor(substr(position,1,1),levels = c("D", "M","F"), ordered = T),
