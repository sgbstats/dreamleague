...
data3=data2%>% 
  ...
  merge(gk %>% janitor::clean_names() %>% dplyr::select(-total), by="team") %>% 
  dplyr::select(num,pos, player.x, player.y, club, team,league, total, notes,player_id,dist, check_needed ) %>% 
  arrange(num)
