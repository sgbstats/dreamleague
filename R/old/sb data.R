...
tictoc::toc()
team_id = cbind.data.frame(team, id) %>%
  filter(
    !is.na(team),
    team != "Year Formed",
    team != "",
    id %notin% c(1821, 2123)
  )
alias = readxl::read_excel("data/DL2324.xlsx", sheet = "Alias")
team_id = team_id %>%
  filter(id %notin% c(2123, 1821, 1895)) %>%
...
for (i in teams_id$id) {
  ...
  out = out %>%
    rbind.data.frame(players %>% mutate(team_id = i, Team = teams$Team[i]))
}
...
fw2 = fw %>%
  ...
  mutate(
    Player = stringi::stri_trans_general(str = Player, id = "Latin-ASCII")
  ) %>%
